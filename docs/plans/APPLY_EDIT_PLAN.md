...
- **Emission**: `pipeline.rs::locate_method_body()` returns `(text, byte-offset-within-capture)` — C# attribute-strip aware via pointer arithmetic (stripping is prefix-only) — and the Edit-fidelity emission threads the absolute span `capture.start_byte + offset .. capture.end_byte`. The old text-only `extract_method_body` wrapper was removed as dead code.
  **Post-launch correction (2026-08-25 regression):** bodies are BRACE-DELIMITED. The original implementation backed up to the start of the line when `{` sat alone on its line (Allman style), embedding leading indentation in the tracked body and permanently rejecting natural `{`..`}` agent extractions (reported as "expected 1018 bytes, actual 1048" on an LF-only file). The line-start branch was removed; text/spans start AT the opening brace. See `src/tests/edit/spans.rs`.
