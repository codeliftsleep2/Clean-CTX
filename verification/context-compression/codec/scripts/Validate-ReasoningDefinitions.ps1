...
$oracles = @(Get-Content -Raw (Join-Path $definitionRoot "codec\oracles.json") | ConvertFrom-Json -Depth 100)
$transport = @(Get-Content -Raw (Join-Path $definitionRoot "codec\transport-assertions.json") | ConvertFrom-Json -Depth 100)
