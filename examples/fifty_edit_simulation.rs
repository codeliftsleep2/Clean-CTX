...
fn generate_edit_sequence() -> Vec<(String, EditCategory, String)> {
    ...
    results.push((
        "Change verifyEmail return type to detailed object".to_string(),
        EditCategory::Method,
        current.clone(),
    ));
    ...
    results.push((
        "Split validateUserData into validateRequiredFields and validateOptionalFields".to_string(),
        EditCategory::Structural,
        current.clone(),
    ));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (description, category, source)) in edits.iter().enumerate() {
        ...
        let compressed_output = compress_file(
            file_path.clone(),
            &mut dict,
            &mut cache,
            Fidelity::Low,
    ...
    println!(
        "Edit{:>4} │ {:<55} │ {:>7} │ {:>7} │ {:>7} │ {:>8} │ {:>8} │ {:>8}",
        "#", "Description", "Raw", "ReComp", "Delta", "CumRaw", "CumReC", "CumDel"
    );
    println!("{}", "─".repeat(145));
    ...
    let full_vs_raw_pct = if total_raw > 0 {
        (full_vs_raw_saved as f64 / total_raw as f64) * 100.0
    } else {
        0.0
    };
    let delta_vs_raw_pct = if total_raw > 0 {
        (delta_vs_raw_saved as f64 / total_raw as f64) * 100.0
    } else {
        0.0
    };
    let delta_vs_full_pct = if total_recomp > 0 {
        (delta_vs_full_saved as f64 / total_recomp as f64) * 100.0
    } else {
        0.0
    };
    ...
    println!(
        "    Raw (no compression):               {:>10} tokens",
        total_raw
    );
    println!(
        "    Clean-CTX full recompression:       {:>10} tokens",
        total_recomp
    );
    println!(
        "    Clean-CTX + delta transport:        {:>10} tokens",
        total_delta
    );
    println!();
    println!("  Savings:");
    println!(
        "    Full compression vs Raw:            {:>10} tokens ({:>5.1}%)",
        full_vs_raw_saved, full_vs_raw_pct
    );
    println!(
        "    Delta vs Raw:                       {:>10} tokens ({:>5.1}%)",
        delta_vs_raw_saved, delta_vs_raw_pct
    );
    println!(
        "    Delta vs Full recompression:        {:>10} tokens ({:>5.1}%)",
        delta_vs_full_saved, delta_vs_full_pct
    );
    println!();
    ...
    for (cat, label) in &categories {
        ...
        let cat_re_save = if cat_raw > 0 {
            (cat_raw.saturating_sub(cat_recomp)) as f64 / cat_raw as f64 * 100.0
        } else {
            0.0
        };
        let cat_del_save = if cat_raw > 0 {
            (cat_raw.saturating_sub(cat_delta)) as f64 / cat_raw as f64 * 100.0
        } else {
            0.0
        };
        println!(
    ...
    match break_even_edit {
        Some(n) => {
            ...
            println!(
                "  ✓ Break-even: Edit #{} — delta transport cumulative ({}) <= full recompression ({})",
    ...
    for rec in &records {
        if rec.raw_cost > 0 {
            let saving =
                (rec.raw_cost.saturating_sub(rec.delta_cost)) as f64 / rec.raw_cost as f64 * 100.0;
    ...
    println!(
        "  ✓ Largest single-edit delta saving vs raw: Edit #{} — {:.1}% reduction",
    ...
    println!(
        "  ✗ Smallest single-edit delta saving vs raw: Edit #{} — {:.1}% reduction (worst case)",
    ...
    for rec in &records {
        if rec.recompression_cost > 0 {
            let saving = (rec.recompression_cost.saturating_sub(rec.delta_cost)) as f64
                / rec.recompression_cost as f64
                * 100.0;
    ...
    println!(
        "  ✓ Largest delta saving vs full recompression: Edit #{} — {:.1}%",
        max_delta_vs_full.0, max_delta_vs_full.1
    );
    println!(
        "  ✗ Smallest delta saving vs full recompression: Edit #{} — {:.1}%",
    ...
    println!();
    println!(
        "  Simulation completed in {:.2}s across {} edits",
        elapsed_secs, 50
    );
