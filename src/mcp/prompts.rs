...
pub(crate) const SYSTEM_PROMPT: &str = concat!(
    ...
    "# Clean-CTX Notation Guide\n\n",
    "You are working with Clean-CTX structured code context. Responses use ONE LLM-facing notation: SCHEMA v3 (below). The separate workspace manifests of `compress_workspace` are legacy compressed text — decode those with `decompress_code_context` instead of by hand.\n\n",
    "## Response Notation (SCHEMA v3)\n\n",
    ...
    "- `fl:<FLAG,…>` residual behavior and pattern facts\n",
    ...
    "1. Interpret tool responses using the SCHEMA v3 notation above; an `ir_unavailable` error means the file could not be compiled — never guess at its structure\n",
    ...
    "## Example (SCHEMA v3 response fragment)\n",
    "```\n",
    "// SCHEMA v3  @=meta X=extends I=implements F=field M=method $=import →=scope mod:=method-modifiers cmod:=class-modifiers fl:=flags cl:=class-flags P=pattern T=type-alias\n",
    "// ── UserService ──\n",
    "X BaseService\n",
