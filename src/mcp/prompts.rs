...
pub(crate) const SYSTEM_PROMPT: &str = concat!(
    ...
    "# Clean-CTX Notation Guide\n\n",
    "You are working with Clean-CTX structured code context. Responses use TWO notations depending on the surface: the PRIMARY response notation (SCHEMA v2 — every provide/compress/restore response below) and a LEGACY compressed-text notation produced only by the workspace/text-delta tools (scoped section below).\n\n",
    ...
    "## Legacy Notation (text-compressor pipeline)\n\n",
    "The vocabulary below is produced ONLY by the secondary text compressor — `compress_workspace` manifests and `delta_text_context` baselines — and consumed by `decompress_code_context`. It does NOT appear in provide_code_context / compress_code_context responses (those use SCHEMA v2 above). It is fidelity-dependent: ⊕ behavior markers appear at Medium/High; Low replaces them with § micro-codes (§I=⊕guard §L=⊕loop §E=⊕⇒ §P=$ctor §C=braces) and additionally encodes frequent tokens as custom symbols ($1, $2, … — see §SYM footer).\n\n",
    "### Primitive opcodes (legacy text output)\n\n",
    ...
    "1. Interpret responses using the notation sections above: SCHEMA v2 for provide/compress/restore responses; the legacy tables only for compress_workspace / delta_text output\n",
