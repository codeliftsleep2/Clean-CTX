...
"#;
    ...
    let class_count = ir.instructions.iter().filter(|op| {
        matches!(op, crate::ir::opcodes::CoreOp::DefClass(_, _))
    }).count();
    
    assert_eq!(class_count, 3, "All 3 classes should be in IR without CBM filter");
}
...
fn cbm_proxy_fallback_on_invalid_json() {
    ...
    assert!(!compressed_invalid.is_empty(), 
