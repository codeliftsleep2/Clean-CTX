...
fn huffman_count_and_build_codes() {
    ...
    for _ in 0..10 { dict.count("Service"); }
    for _ in 0..5 { dict.count("Observable"); }
    for _ in 0..2 { dict.count("HttpClient"); }
    ...
    assert!(dict.get_opcode("Service").is_some(), "Service should get a code");
    assert!(dict.get_opcode("Observable").is_some(), "Observable should get a code");
    assert!(dict.get_opcode("HttpClient").is_some(), "HttpClient should get a code");
    ...
    assert!(service_code < observable_code,
...
fn huffman_encode_replaces_tokens() {
    let mut dict = HuffmanSymbolDictionary::new();
    for _ in 0..5 { dict.count("Service"); }
    for _ in 0..3 { dict.count("Observable"); }
    dict.build_codes();

    let encoded = dict.encode("Service Observable Service");
    // Both should be replaced with short codes
    assert_ne!(encoded, "Service Observable Service", "Should replace tokens");
    assert!(!encoded.contains("Service"), "Service should be replaced");
    assert!(!encoded.contains("Observable"), "Observable should be replaced");
}
...
fn huffman_footer_format() {
    ...
    assert!(footer.contains("Observable"), "Footer should contain Observable");
    // Should have frequency counts
    assert!(footer.contains("(10)"), "Footer should show count 10 for Service");
    assert!(footer.contains("(5)"), "Footer should show count 5 for Observable");
}
...
fn huffman_deterministic_output() {
    ...
    let make_dict = || {
        let mut dict = HuffmanSymbolDictionary::new();
        for _ in 0..10 { dict.count("Alpha"); }
        for _ in 0..5 { dict.count("Beta"); }
        for _ in 0..3 { dict.count("Gamma"); }
        dict.build_codes();
        dict
    };
...
fn huffman_skips_single_char_tokens() {
    ...
    assert!(dict.get_opcode("ab").is_some(), "Two-char token should get a code");
}

#[test]
fn huffman_total_frequency() {
    let mut dict = HuffmanSymbolDictionary::new();
    for _ in 0..7 { dict.count("Service"); }
    for _ in 0..3 { dict.count("Observable"); }
