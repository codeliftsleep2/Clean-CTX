...
//
// WorkspaceIndex core tests: insertion, occurrence-aware edge
// deduplication, entity ambiguity, file provenance, and counters.
//
// Shared edge-construction helpers live in `index_support`; graph traversal
// tests live in `index_graph`; name/inject/selector resolution tests live in
// `index_queries`; same-name occurrence-collision regressions live in
// `index_edge_occurrence`.
//
...
#[test]
fn edge_occurrence_dedup_preserves_cross_file_evidence() {
    let mut idx = WorkspaceIndex::new();
    ...
    // Two edge occurrences: the semantic triple (relation + subject identity +
    // object identity) is identical, but the asserting source occurrence
    // differs, so neither may suppress the other.
    assert_eq!(
        idx.edge_count(),
        2,
        "the same triple asserted by two files is two occurrences"
    );
    assert_eq!(idx.total_edges_inserted(), 2);
    // Entity occurrences retained from BOTH files (architectural review fix).
    // Entity registration happens before edge dedup, so both files'
    ...
    // Each asserting occurrence keeps its own evidence in both indexes.
    assert_eq!(
        idx.forward_edges_by_identity("angular", "Component", "UserComponent")
            .len(),
        2,
        "both files' outgoing evidence must survive"
    );
    assert_eq!(
        idx.reverse_edges_by_identity("angular", "Service", "UserService")
            .len(),
        2,
...
fn counters_reflect_insertion_and_dedup() {
    ...
    // 2 duplicates of edges already asserted by THIS file — deduplicated.
    idx.add_edges(
        "a.ts",
        vec![
            inject_edge("A", "S", Some("a.ts")),
    ...
    assert_eq!(idx.edge_count(), 3, "same-file duplicates must dedup");
    ...
    // A different file asserting an already-indexed triple adds an occurrence.
    idx.add_edges("c.ts", vec![route_edge("/admin", "Admin")]);
    assert_eq!(idx.total_edges_inserted(), 7);
    assert_eq!(
        idx.edge_count(),
        5,
        "cross-file occurrences of one triple must be counted separately"
    );
}
