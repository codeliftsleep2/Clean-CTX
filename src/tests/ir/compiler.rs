...
fn compile_edit_focused(focus: Option<&HashSet<String>>) -> CompiledIR {
    ...
    let mut compiler = IRCompiler::new();
    compiler
        .compile_focused(source, "test_edit_focused", language, query, Fidelity::Edit, None, focus)
        .expect("compilation should succeed")
}
...
fn compile_focused_none_emits_all_bodies() {
    ...
    assert_eq!(bodies.len(), 3, "all 3 methods should have bodies, got {:?}", bodies);
...
fn compile_focused_some_extracts_only_named_bodies() {
    ...
    assert_eq!(
        bodies.len(), 1,
        "only 1 body should be extracted, got {:?}", bodies
    );
    assert!(bodies.contains(expected_mid), "processComplexData body missing");
}
...
fn compile_focused_multiple_names_extracts_only_named_bodies() {
    let focus: HashSet<String> = ["processComplexData".into(), "healthCheck".into()].into_iter().collect();
    let ir = compile_edit_focused(Some(&focus));
    let names = method_names_by_id(&ir);
    let bodies = body_method_ids(&ir);
    ...
    assert_eq!(
        bodies.len(), 2,
        "2 methods should have bodies, got {:?}", bodies
    );
...
fn compile_focused_non_edit_fidelity_ignores_focus() {
    ...
    let mut compiler = IRCompiler::new();
    let ir = compiler
        .compile_focused(source, "test_high_focused", language, query, Fidelity::High, None, Some(&focus))
        .expect("compilation should succeed");
    let bodies = body_method_ids(&ir);
    assert!(
        bodies.is_empty(),
