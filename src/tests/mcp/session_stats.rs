...
use crate::mcp::session_stats::{SessionStats, render_dashboard_json, render_dashboard_text};
...
fn test_record_cbm_proxy_accumulates() {
    ...
    let summary = stats.summary();
    assert_eq!(
        summary.total_raw_tokens, 15000,
        "should accumulate raw tokens across calls"
    );
    assert_eq!(
        summary.total_compressed_tokens, 3300,
        "should accumulate compressed tokens across calls"
    );
    assert_eq!(
        summary.full_compress_count, 3,
        "each CBM call is a full compression event"
    );

    // Per-tool file entry should accumulate
    let fs = stats
        .file_stats("cbm://graph_search")
        .expect("cbm://graph_search should be tracked");
...
fn test_record_cbm_proxy_distinct_tools() {
    ...
    assert_eq!(
        ...
        "two distinct CBM tools = two file entries"
    );
...
fn test_merge_preserves_cbm_proxy_event_count() {
    ...
    assert_eq!(
        ...
        5000 * 3 + 3000 * 2,
        "raw tokens accumulate across merge"
    );
    assert_eq!(
        summary.total_compressed_tokens,
        1100 * 3 + 800 * 2,
        "compressed tokens accumulate across merge"
    );
...
fn test_render_dashboard_shows_angular_template_domain() {
    ...
    assert!(
        text.contains("917"),
        "dashboard should show raw token count"
    );
    assert!(
        text.contains("300"),
        "dashboard should show compressed token count"
    );
...
fn test_multiple_records_same_file() {
    ...
    assert!(
        fs.has_llm_savings(),
        "delta after full should preserve LLM savings"
    );
...
fn test_delta_efficiency_when_delta_larger_than_full() {
    ...
    assert_eq!(fs.strategy, "delta");
    assert!(
        fs.delta_efficiency_pct.is_none(),
        "no savings when delta > full"
    );
}
...
fn test_render_dashboard_json_delta_fields() {
    ...
    assert!(
        file["savings_pct"].is_null(),
        "delta file savings should be null"
    );
    assert_eq!(
        file["delta_efficiency_pct"], 70.0,
        "efficiency: (300-90)/300=70%"
    );
    assert_eq!(file["full_compressed_tokens"], 300);
    assert!(
        !json["session"]["avg_delta_efficiency_pct"].is_null(),
...
fn test_merge_in_memory_wins_for_existing_file() {
    ...
    assert_eq!(
        shared.compressed_tokens, 1000,
        "in-memory compressed_tokens should win"
    );
...
fn test_merge_db_only_file_imported() {
    ...
    assert!(
        in_memory.file_stats("/test/db_only.ts").is_some(),
        "DB-only file should appear"
    );
...
fn test_strategy_labels() {
    ...
    assert_eq!(
        stats.file_stats("/test/f3.cs").unwrap().strategy,
        "workspace"
    );
    assert_eq!(
        stats.file_stats("/test/f4.js").unwrap().strategy,
        "workspace_gsym"
    );
...
fn test_multiple_files_in_dashboard() {
    let mut stats = SessionStats::new();
    let files = vec!["/src/a.ts", "/src/b.rs", "/src/c.ts", "/src/d.cs"];
    ...
    for f in &files {
        ...
        assert!(
            text.contains(&display[..std::cmp::min(display.len(), 10)]),
...
fn test_sync_cache_metrics_populates_prompt_cache_domain() {
    ...
    assert!(
        domain.is_some(),
        "prompt_cache domain should exist after sync"
    );
...
fn test_sync_cache_metrics_accumulates_existing() {
    ...
    assert_eq!(
        domain.cache_hits,
        Some(13),
        "hits should accumulate across syncs"
    );
    assert_eq!(
        domain.cache_misses,
        Some(5),
        "misses should accumulate across syncs"
    );
...
fn test_sync_cache_metrics_preserves_proxy_hits() {
    ...
    assert_eq!(
        domain.total_raw_tokens, 6200,
        "proxy tokens must be preserved"
    );
}
...
fn test_dashboard_text_includes_prompt_cache_after_sync() {
    ...
    assert!(
        ...
        "dashboard should show Prompt Cache domain"
    );
    assert!(
        text.contains("2,100"),
        "dashboard should show tokens saved from cache"
    );
}
...
fn test_delta_preserves_full_compression_savings() {
    ...
    assert_eq!(
        fs.raw_tokens, 1000,
        "raw_tokens should be preserved from full compression"
    );
    assert_eq!(
        fs.compressed_tokens, 250,
        "compressed_tokens should be preserved from full compression"
    );
    assert!(
        (fs.savings_pct - 75.0).abs() < 0.01,
        "savings_pct should be preserved"
    );
    assert!(
        fs.has_llm_savings(),
        "delta file with preserved savings should still show LLM savings"
    );
    assert_eq!(fs.strategy, "delta");
    assert_eq!(fs.delta_count, 1);
    ...
    let summary = stats.summary();
    assert_eq!(
        summary.total_raw_tokens, 1000,
        "session raw should reflect full compression"
    );
    assert_eq!(
        summary.total_compressed_tokens, 250,
        "session compressed should reflect full compression"
    );
    assert!(
        (summary.total_savings_pct - 75.0).abs() < 0.01,
        "session savings should be preserved"
    );
...
fn test_delta_preserves_savings_in_dashboard() {
    ...
    assert!(
        text.contains("75.0%"),
        "dashboard should show 75.0% savings, got: {}",
        text
    );
    assert!(
        !text.contains("N/A"),
        "dashboard should NOT show N/A for a delta file with preserved savings"
    );
    ...
    let file = &json["files"][0];
    assert!(
        file["savings_pct"].is_number(),
        "JSON savings_pct should be numeric, got: {}",
        file["savings_pct"]
    );
...
fn test_delta_without_prior_full_still_shows_na() {
    ...
    assert!(
        !fs.has_llm_savings(),
        "delta with no prior full should NOT have LLM savings"
    );
