...
fn test_context_store_round_trip() {
    ...
    let id = store.save_context("/test/file.ts", Fidelity::Low, "compressed", None, "abc123", 1000, 250)
...
fn test_has_context() {
    ...
    store.save_context("/test/file.ts", Fidelity::Medium, "compressed", None, "hash1", 1000, 250)
        .expect("save should succeed");
    assert!(store.has_context("/test/file.ts"));
}
...
fn test_clear_file_removes_context() {
    let mut store = InMemoryContextStore::new();
    store.save_context("/test/file.ts", Fidelity::Low, "compressed", None, "hash1", 1000, 250)
        .expect("save should succeed");
    assert!(store.has_context("/test/file.ts"));
...
fn test_delta_count() {
    let mut store = InMemoryContextStore::new();
    let id = store.save_context("/test/file.ts", Fidelity::Low, "compressed", None, "hash1", 1000, 250)
        .expect("save should succeed");
...
fn test_save_context_with_token_counts() {
    let mut store = InMemoryContextStore::new();
    let _id = store.save_context("/test/file.ts", Fidelity::Low, "compressed", None, "hash1", 5000, 800)
        .expect("save should succeed");
    let meta = store.load_latest("/test/file.ts").expect("load should succeed").unwrap();
